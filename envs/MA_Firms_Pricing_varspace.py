...
# -*- coding: utf-8 -*-
"""
Created on Mon Jun 17 16:19:06 2019
...
class MultiAgentFirmsPricing(MultiAgentEnv):
    ...
    def __init__(self, env_config={
        ...
        self.observation_space = gym.spaces.Discrete(self.p_num**self.num)
        
        actions_space = list(range(self.p_num))
        possible_states = [p for p in itertools.product(actions_space, repeat=self.num)]
        index_states = list(range(self.p_num**self.num))
        self.s_dict = dict(zip(possible_states, index_states))
        
        # Create list of agents and dictionary of initial observation
        self.agents = list()
        self.obs = dict()
        self.info = dict()
        obs = np.random.randint(self.p_num**self.num)
        
        # Name agents 'agent_0', 'agent_1' and so on
        # and initialize observation to nash equilibrium
        for i in range(self.num):
            self.agents.append('agent_'+str(i))
            self.obs.update({'agent_'+str(i):obs})
